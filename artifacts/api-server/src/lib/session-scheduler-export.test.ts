...
describe("buildSessionSchedulerExportRows", () => {
  it("includes only paid and invoiced bookings and excludes TBC or unusable emails", () => {
    ...
    const attendees = [
      ...
      attendee({ id: 108, bookingId: 1, isTbc: true, workEmail: "tbc@example.com" }),
      attendee({ id: 109, bookingId: 1, workEmail: "not-an-email" }),
      attendee({ id: 110, bookingId: 1, workEmail: "seat-2@tbc.placeholder" }),
    ];
  ...
  it("normalises output and keeps the latest case-insensitive email match", () => {
    const older = new Date("2026-08-20T08:00:00Z");
    const latest = new Date("2026-08-20T10:00:00Z");
  ...
  it("sorts by name and email and grows automatically when live input gains an attendee", () => {
...
describe("Session Scheduler workbook", () => {
  ...
  it("uses the required London-date filename", () => {
    expect(getSessionSchedulerExportFilename(new Date("2026-08-20T12:00:00Z"))).toBe(
      "hras26-session-scheduler-attendees-2026-08-20.xlsx",
    );
  });
});
