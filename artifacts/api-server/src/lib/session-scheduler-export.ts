...
export function createSessionSchedulerWorkbook(
  ...
  headerRow.fill = { type: "pattern", pattern: "solid", fgColor: { argb: "FFE74F3E" } };
  ...
  sheet.eachRow((row, rowNumber) => {
    ...
    if (rowNumber % 2 === 0) {
      row.eachCell((cell) => {
        cell.fill = { type: "pattern", pattern: "solid", fgColor: { argb: "FFFCFBFA" } };
      });
    }
  });
...
export function getSessionSchedulerExportFilename(date = new Date()): string {
  ...
  return `hras26-session-scheduler-attendees-${part("year")}-${part("month")}-${part("day")}.xlsx`;
}
