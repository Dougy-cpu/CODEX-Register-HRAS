...
function registrationDisplayDate(b: typeof bookingsTable.$inferSelect): Date {
  const isConfirmed = b.status === "paid" || b.status === "invoiced" || b.status === "transferred";
...
router.post(
  ...
  async (req, res): Promise<void> => {
    ...
    for (const attendee of welcomeAttendees) {
      const ok = await sendWelcomeEmail(id, attendee.firstName, attendee.workEmail);
      if (ok) {
