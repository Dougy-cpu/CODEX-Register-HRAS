...
describe("admin audit trail — integration", () => {
  ...
  it("GET /admin/registrations/export/scheduler generates fresh, unfiltered attendee rows", async () => {
    ...
    expect(firstResponse.headers.get("content-disposition")).toMatch(
      /hras26-session-scheduler-attendees-\d{4}-\d{2}-\d{2}\.xlsx/,
    );
  ...
  it("keeps the existing detailed registrations export contract unchanged", async () => {
    seedBooking({ id: 101, status: "paid", orderReference: "HRAS-12345" });
    seedBooking({ id: 102, status: "partial", orderReference: "HRAS-12346" });
    ...
    expect(Array.from({ length: 22 }, (_, index) => sheet?.getCell(1, index + 1).value)).toEqual([
      "Booking Reference",
      "Status",
      "Pass Type",
      "Qty",
  ...
  it("Community Social send failure leaves the manual delivery flag false", async () => {
    ...
    sendCommunitySocialEmailMock.mockResolvedValue(false);
    ...
    expect(res.status).toBe(502);
    const booking = getRows({ _name: "bookingsTable" })[0];
    expect(booking.communitySocialEmailSent).toBe(false);
    expect(runConfirmationSideEffectsMock).not.toHaveBeenCalled();
  });
